    ...
    def run(self) -> None:
        while not self._stop.is_set():
            try:
                ...
                action, source = cmd
                try:
                    getattr(self.backend, action)(source)
...
class PlayerApp:
    ...
    def _build(self) -> None:
        ...
        self.cover = tk.Label(self.root, bg=BG, image=self._placeholder, bd=0)
        self.cover.pack(pady=(10, 14))

        self.title_lbl = tk.Label(self.root, text="Ничего не играет", bg=BG, fg=FG,
                                  font=("Segoe UI", 15, "bold"), wraplength=330, justify="center")
        self.title_lbl.pack(padx=20)
        self.artist_lbl = tk.Label(self.root, text="Включите музыку в браузере или приложении",
                                   bg=BG, fg=MUTED, font=("Segoe UI", 11), wraplength=330, justify="center")
        self.artist_lbl.pack(padx=20, pady=(2, 0))
        self.album_lbl = tk.Label(self.root, text="", bg=BG, fg=MUTED, font=("Segoe UI", 9),
        ...
        self.root.bind("<space>", lambda e: self._send("play_pause"))
        self.root.bind("<Right>", lambda e: self._send("next"))
        self.root.bind("<Left>", lambda e: self._send("previous"))
    ...
    def _send(self, action: str) -> None:
        if self.current:
            self.poller.commands.put((action, self.current.source_id))
        ...
        try:
            while True:
                ...
                if kind == "sessions":
                    ...
                    self.status_lbl.config(text=f"Бэкенд: {self.backend.name} · источников: {len(payload)}")
                    self._apply()
                else:
...
def run_gui(backend: MediaBackend, interval: float = 1.0) -> None:
    PlayerApp(backend, interval).run()
